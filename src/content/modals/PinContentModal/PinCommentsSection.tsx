...
import InfiniteScroll from '@/content/components/InfiniteScroll'
...
export function PinCommentsSection({ pinId }: { pinId: string }) {
  ...
  return (
    <Box sx={styles.commentsRoot}>
      <Typography variant="subtitle1" sx={styles.sectionTitle}>
        Comments {comments.count > 0 ? `(${comments.count})` : ''}
      </Typography>

      <Box component="form" onSubmit={handleSubmit} sx={styles.addCommentForm}>
        <StyledTextField
          ...
          disabled={submitting}
        />
        <Button type="submit" variant="contained" disabled={!newContent.trim() || submitting} sx={styles.addCommentBtn}>
      ...
      <Box sx={styles.commentsListWrap}>
        <InfiniteScroll onYEnd={handleLoadMore} isLoading={isLoading} showLoader={hasMore} sx={styles.commentsScrollSx}>
